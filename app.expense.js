// ---------- Expense module ----------
  ...
  if (summaryEl){
    // 車資：來自訂單（不會顯示在花費列表，但會納入花費統計）
    const travelTotal = (Array.isArray(orders) ? orders : [])
      .filter(o => {
    ...
    if (!list.length && !travelTotal){
      ...
      if (travelTotal){
        byCat['車資'] = (byCat['車資'] || 0) + travelTotal;
      }
      ...
      let text = travelTotal
        ? `本月共有 ${list.length} 筆花費，車資 ${fmtCurrency(travelTotal)}，累計 ${fmtCurrency(grandTotal)}。`
        : `本月共有 ${list.length} 筆花費，累計 ${fmtCurrency(total)}。`;
