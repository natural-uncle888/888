...
// === pill toggle sync: keep label.on in sync with checkbox state ===
(function(){
  function syncPills(root=document){
    const pills = Array.from(root.querySelectorAll('label.pill'));
    pills.forEach(label=>{
      const input = label.querySelector('input[type="checkbox"]');
      if(!input) return;
      // initialize class
      if(input.checked) label.classList.add('on'); else label.classList.remove('on');
      // attach listener
      input.addEventListener('change', ()=> {
        if(input.checked) label.classList.add('on'); else label.classList.remove('on');
      });
      // also make label clickable even if span intercepts clicks
      label.addEventListener('click', (e)=> {
        // do nothing special — letting native label toggle handle input
        // but ensure class toggles (in case change event doesn't fire immediately)
        setTimeout(()=> {
          if(input.checked) label.classList.add('on'); else label.classList.remove('on');
        }, 0);
      });
  ...
  // expose a function if dynamic content needs it
  window.syncPills = syncPills;
})();


/* ===== Reminders pill initializer & toggle sync =====
   This script will:
   - Convert any standalone elements whose innerText is exactly "不再提醒" into a standardized
     <label class="pill danger"><input type="checkbox" ...><span>不再提醒</span></label>
     unless one already exists.
   - Attach click handlers to all .pill labels so clicking anywhere toggles the checkbox.
   - Keep a `.on` class on the label to reflect checked state (useful for CSS styling).
*/
(function(){
  function makePill(labelText, id, isDanger){
    var lbl = document.createElement('label');
    lbl.className = 'pill' + (isDanger ? ' danger' : '');
    // Create checkbox
  ...
  function syncLabelState(lbl){
    var cb = lbl.querySelector('input[type="checkbox"]');
    if(!cb) return;
    if(cb.checked) lbl.classList.add('on'); else lbl.classList.remove('on');
  }

  function attachPillBehavior(lbl){
    if(lbl.__pill_bound) return;
    lbl.__pill_bound = true;
    // Ensure there's a checkbox
    var cb = lbl.querySelector('input[type="checkbox"]');
    if(!cb){
    ...
    lbl.addEventListener('click', function(e){
      // prevent double toggling if click lands on input element itself
    ...
    // Also respond to keyboard change events
    cb.addEventListener('change', function(){ syncLabelState(lbl); });
  }

  function upgradeExistingTextNodes(){
    // Find elements whose trimmed innerText equals "不再提醒" and have no child elements.
    ...
    toReplace.forEach(function(orig){
      // If parent already contains a .pill ancestor, skip
      if(orig.closest && orig.closest('.pill')) return;
      var pill = makePill('不再提醒', 'reminderMuted', true);
      // transfer attributes that may be meaningful (like data-*)
      Array.prototype.slice.call(orig.attributes || []).forEach(function(attr){
        try{ pill.setAttribute(attr.name, attr.value); }catch(e){}
      });
      orig.parentNode.replaceChild(pill, orig);
    });
  }

  function initAllPills(){
    // upgrade standalone text nodes
    upgradeExistingTextNodes();
    // find all .pill labels and attach behavior
    var pills = document.querySelectorAll('label.pill, .pill');
    pills.forEach(function(p){
      // If it's not a label, wrap into a label element (rare)
      if(p.tagName.toLowerCase() !== 'label'){
        var wrapper = document.createElement('label');
        wrapper.className = p.className || 'pill';
        p.parentNode.replaceChild(wrapper, p);
        wrapper.appendChild(p);
        p = wrapper;
      }
      attachPillBehavior(p);
    });
  }

  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', initAllPills);
  } else {
    initAllPills();
  }
})();
